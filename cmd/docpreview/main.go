...
func setup(configPath, logLevel string) (*wiring, error) {
	...
	if cfg.GitHub.AppID != 0 {
		...
		case errors.Is(err, vault.ErrLocked):
			...
			// served by this process, so failing here would make that page
			// unreachable and leave the terminal as the only way in.
			//
	...
	// Bitbucket, on the same terms: a locked vault leaves the client unbuilt and
	// /webhook/bitbucket answering 501, and rewireBitbucket installs one at unlock.
	if cfg.Bitbucket.Enabled {
		v, err := w.Vault()
		switch {
		case errors.Is(err, vault.ErrLocked):
			log.Warn("bitbucket is configured but the vault is locked, so no Bitbucket client was built",
				"fix", "unlock the vault from the dashboard, or set vault.key_source",
				"key_source", cfg.KeySource().Describe(),
				"vault", w.vaultPath)
		case err != nil:
			...
			return nil, err
		default:
			bb, err := bitbucket.New(cfg.Bitbucket, v, log)
			if err != nil {
				// A warning, not a refusal to start, and the difference is
				// load-bearing: the webhook secret is *generated by this daemon* —
				// there is a Generate button on /secrets and an endpoint behind it —
				// so a daemon that will not boot without it cannot be used to create
				// it.
				//
				// The same shape as a locked vault, for the same reason. Until the
				// client exists /webhook/bitbucket answers 501, which is the truth,
				// and rewireBitbucket installs it the moment the keys are stored.
				log.Warn("bitbucket is enabled but its credentials are not complete yet",
					"reason", err,
			...
			//
			// Without it here, the client built at startup can only ever see the
			// workspace-wide credential: on a daemon whose vault is already unlocked
			// (the normal case, with vault.key_source set), every per-project token
			// would be invisible, and Test credential would answer "no Bitbucket
			// credential for owner/repo" about a token sitting in the vault.
...
func (w *wiring) hasSCM() bool {
	return len(w.clients) > 0 || w.cfg.GitHub.AppID != 0 || w.cfg.Bitbucket.Enabled
}
...
func cmdServe(args []string) error {
	...
	// A daemon nothing can reach looks healthy while being inert, but refusing to
	// start is worse: configuring GitHub means storing a private key and a webhook
	// secret from the dashboard, and refusing to boot until that is done would
	// leave the only route back to a terminal — the ceremony the page exists to
	// remove.
	//
	...
	admin = daemon.NewSecretsAdmin(w.cfg, w.log, func(changed string) {
		...
		rewireBitbucket(w, d, ingress, v, changed)
	...
	srv := &http.Server{
		...
		// the full 30-second shutdown timeout and then log a failure.
...
// rewireBitbucket installs the Bitbucket client, on the same terms as GitHub's.
//
// Deferred construction rather than wiring, for the reason the whole rearm mechanism
// exists: the vault may be locked at startup, and the page that unlocks it is served
// by this daemon. Until a client exists, /webhook/bitbucket answers 501 — which is
// the truth, since nothing could verify a signature.
//
// One shape difference from GitHub: which vault keys matter depends on the auth mode,
// so the filter has to know the mode. Rebuilding on a key belonging to the *other*
// mode would discard a working client because an unrelated credential was stored.
func rewireBitbucket(w *wiring, d *daemon.Daemon, ingress *daemon.Ingress, v *vault.Vault, changed string) {
	if !w.cfg.Bitbucket.Enabled {
		return
	}

	interesting := map[string]bool{"": true, vault.KeyBitbucketHookSec: true}
	if w.cfg.Bitbucket.Auth == config.BitbucketAuthAPIToken {
		interesting[vault.KeyBitbucketEmail] = true
		interesting[vault.KeyBitbucketAPIToken] = true
	} else {
		interesting[vault.KeyBitbucketAccessToken] = true
	}
	if !interesting[changed] {
		return
	}

	bb, err := bitbucket.New(w.cfg.Bitbucket, v, w.log)
	if err != nil {
		// Storing one credential and not yet the other is the normal middle of setup,
		// and this fires on the first of them every time. The page shows what is still
		// missing.
...
func cmdDoctor(args []string) error {
	...
	for i, l := range w.cfg.Listeners {
		...
		// Who may write through an overlay listener, otherwise visible only by reading the
		// config file.
		//
	...
	// and the answer is decided by whether a viewer password exists, not only by the listeners
	// above. A daemon behind a public share with no password is a dashboard on the internet,
	// and nothing else in this output says so.
	...
	default:
		...
		if w.cfg.Bitbucket.Enabled {
			// The auth mode is reported either way, because which vault key holds the
			// credential depends on it and an operator debugging a 401 needs to know
			// which one the daemon is actually reading.
			state := ""
			if _, ok := w.clients[model.PlatformBitbucket]; !ok {
				state = ", not wired up: the vault is locked"
			}
			names = append(names, fmt.Sprintf("bitbucket (%s%s)", w.cfg.Bitbucket.Auth, state))
		}
...
//     whose name is something else. Fails startup when the key is missing, because a
//     configured credential that is absent is a misconfiguration.
...
//
// Failing loudly on a missing build.secrets key is deliberate: the alternative is a build
// that runs with the variable unset, produces a site missing whatever the credential was
// for, reports success, and leaves the redactor built from one fewer value than the
// operator believes.
//
// The vault is opened only when there is something to look up, so a server with no build
// secrets still starts without a passphrase.
//
